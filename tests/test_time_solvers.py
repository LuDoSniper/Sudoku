...
recursif = timeit.timeit(lambda: backtracking_recursif(grid, use_heuristic=False), number=1)
print(f"Temps d'exécution pour le recursif: {recursif:.2f} secondes")

# iteratif = timeit.timeit(lambda: backtracking_iteratif_pile(grid, use_heuristic=True), number=1)
